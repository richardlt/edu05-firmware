...
void ProcessIO(void){   
	...
	//Check if we have received an OUT data packet from the host
		...
		switch(ReceivedDataBuffer[0]){ //Look at the data the host sent, to see what kind of application specific command it sent.
			...
			case 0x03:{ //ReadAnalogChannel()
				if(!HIDTxHandleBusy(USBInHandle)){
					getAnalogInput(ReceivedDataBuffer[1], &ToSendDataBuffer[3], &ToSendDataBuffer[2]);
			...
			case 0x0B:{	//LCDWriteString()
                ...
                if (ReceivedDataBuffer[1]>=32) {
					index=ReceivedDataBuffer[1]/32;
					index=ReceivedDataBuffer[1]-index*32;
				} else {
                    index=ReceivedDataBuffer[1];
                }
                ...
                lcdSendConfigByte(cursor);
                
                while(i<ReceivedDataBuffer[2]){
                    ...
                    lcdSendByte(ReceivedDataBuffer[3+i]);
                    i++;
                    index++;
                }
